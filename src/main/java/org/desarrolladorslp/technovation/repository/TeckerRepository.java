...
public interface TeckerRepository extends JpaRepository<TeckerAssignment, UUID> {
    ...
    @Query("SELECT new org.desarrolladorslp.technovation.models.DeliverableByTecker(ta.id, d.title, d.dueDate, ta.status) FROM TeckerAssignment ta INNER JOIN Deliverable d ON ta.deliverableId = d.id WHERE ta.teckerId = :teckerId")
